...
namespace System\Handlers\Admin\System {
	...
	class Settings extends System\Frames\Admin\Handler {
		...
		protected function handle() {
			...
			$fieldset->input        (CONFIG_PARAM_SITE_TITLE, CONFIG_SITE_TITLE, FORM_INPUT_TEXT, CONFIG_SITE_TITLE_MAX_LENGTH);

			$fieldset->select       (CONFIG_PARAM_SITE_STATUS, CONFIG_SITE_STATUS, Lister\Status::range());

			$fieldset->input        (CONFIG_PARAM_SITE_DESCRIPTION, CONFIG_SITE_DESCRIPTION, FORM_INPUT_TEXTAREA, CONFIG_SITE_DESCRIPTION_MAX_LENGTH);

			$fieldset->input        (CONFIG_PARAM_SITE_KEYWORDS, CONFIG_SITE_KEYWORDS, FORM_INPUT_TEXTAREA, CONFIG_SITE_KEYWORDS_MAX_LENGTH);

			$fieldset->input        (CONFIG_PARAM_SYSTEM_URL, CONFIG_SYSTEM_URL, FORM_INPUT_TEXT, CONFIG_SYSTEM_URL_MAX_LENGTH);

			$fieldset->select       (CONFIG_PARAM_SYSTEM_TIMEZONE, CONFIG_SYSTEM_TIMEZONE, Timezone::range());

			$fieldset->input        (CONFIG_PARAM_SYSTEM_EMAIL, CONFIG_SYSTEM_EMAIL, FORM_INPUT_TEXT, CONFIG_SYSTEM_EMAIL_MAX_LENGTH);
