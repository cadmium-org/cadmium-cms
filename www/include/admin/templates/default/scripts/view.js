...
var View = {
	...
	'initComponents' : function(parent) {
		...
		parent.find('.fixed.button').popup({ 'position': 'left center', 'variation': 'inverted' }).transition('jiggle');

		parent.find('.icon.button').popup({ 'position': 'bottom center', 'variation': 'inverted' });
		...
		parent.find('.ui.checkbox').checkbox();

		parent.find('.ui.accordion').accordion();
